...
describe("Notify.vue", () => {
  ...
  it("Snackbar has error message", async done => {
    ...
    flushPromises().then(() => {
      ...
      expect(wrapper.find(".v-snack__wrapper").element.classList).toContain("error");
      done();
    });
  });
  ...
  it("Snackbar has warning message", async done => {
    ...
    flushPromises().then(() => {
      expect(wrapper.find(".v-snack__content").text()).toBe(msg)
      expect(wrapper.find(".v-snack__wrapper").element.classList).toContain("warning");
      done();
    });
  });
  ...
  it("Snackbar has success message", async done => {
    ...
    flushPromises().then(() => {
      expect(wrapper.find(".v-snack__content").text()).toBe(msg)
      expect(wrapper.find(".v-snack__wrapper").element.classList).toContain("success");
      done();
    });
  });
  ...
  it("Snackbar has info message", async done => {
    ...
    flushPromises().then(() => {
      expect(wrapper.find(".v-snack__content").text()).toBe(msg)
      expect(wrapper.find(".v-snack__wrapper").element.classList).toContain("info")
      done()
    });
  });
});
